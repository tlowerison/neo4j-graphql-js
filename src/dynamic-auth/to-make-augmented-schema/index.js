...
const codifyDirectiveInstance = ({ name, args }) =>
  [
    name,
...
const decodifyDirectiveInstance = codifiedDirectiveInstance => {
...
const getCustomDirectiveInstances = config => {
  ...
  const calledInstances = calledCustomDirectiveInstances
    ...
    .map(({ index, value, end }) => {
      ...
      return {
        ...
        args: fromPairs(
          unnest(
            customDirectives[directiveValue].map(({ name }) =>
              zip(
